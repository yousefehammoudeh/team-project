...
| ID  | User Story (acceptance summary)                                                                                                           | Owner  | Status | PR(s) [with associated user story tag]                     |
|-----|-------------------------------------------------------------------------------------------------------------------------------------------|--------|--------|------------------------------------------------------------|
| S01 | **Create Room (Host):** generate room code; persist room; show host dashboard (CreatedRoomView).                                          | Diana  | [x]    | 26, 41, 52, 68, 74, 76, 93                                 |
| S02 | **Join Room (Participant):** enter code + name; join and see participant dashboard (CreatedRoomView) with room code and participant list. | Elaine | [x]    | 39, 40, 54, 55, 57, 60, 62, 64, 71, 76, 81, 89, 103, 107   |
| S03 | **Search & Details:** search TMDB; show list (title/year/poster); open details panel.                                                     | Tamako | [x]    | 28, 38, 62, 72, 73, 75, 76, 77, 82, 94, 95, 104            |
| S04 | **Build & Lock Shortlist (Host):** add/remove until lock.                                                                                 | He Sun | [x]    | 27, 28, 47, 51, 62, 65, 66, 67, 69, 71, 76, 78, 83, 85, 88 |
| S05 | **Vote & Winner:** participants submit ranked ballots; host computes and displays winner.                                                 | Yousef | [x]    | 34, 37, 62, 76, 96, 102                                    |
...
3) View Infrastructure & Switching [Easy]
 ...
 - [x] Final polish: finalize helpers, constants and ensure smooth switching logic (in-progress)
...
- [x] Implement method logic and additional persistence helpers (in-progress)
...
- [x] Complete and stabilize all InputBoundary/InputData/OutputData contracts for every story (in-progress)

7) App Composition Wiring [Medium]
- [x] `app/AppBuilder.java` and `app/Main.java` exist and provide composition entry points
- [x] Final wiring, dependency injection and startup ordering need validation (in-progress)
...
- [x] Finish consistent `present`/`presentFailure` implementations and event firing (in-progress)
...
10) S02 Join Room — Implementation + Tests [Challenging]
 - [x] S02 Join Room — implementation in progress (JoinRoomView and ParticipantsDashboardView created; interactor and wiring needed)

11) S04 Shortlist — Implementation + Tests [Challenging]
 - [x] S04 Shortlist — implementation in progress (shortlist presenter and controllers present)

12) S03 Search & Details — Stub, Then TMDB Gateway [Challenging]
 - [x] S03 Search & Details — TMDB gateway class present (`data_access/tmdb/TmdbMovieGateway.java`); wiring and tests remain to be completed

13) S05 Vote & Winner — Implementation + Tests [Challenging]
 - [x] S05 Vote & Winner — controllers and presenter classes exist; winner computation and tests pending

14) Polish, UX, and Integration Plan [Medium]
- [x] Polish, UX, and Integration — ongoing; some unit tests and UI tests exist under `test/` (shortlist, host dashboard, participants dashboard, vote)
